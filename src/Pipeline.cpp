...
Pipeline::Pipeline(bool waitForSlowestFilter,
                   bool useThreading,
                   int msecsBetweenFrames)
    : m_lastPipeID(0)
    , m_waitForSlowestFilter(waitForSlowestFilter)
    , m_bUseThreading(useThreading)
    , m_msecsBetweenFrames(msecsBetweenFrames)
    , m_nrOfSources(0)
{}
...
void Pipeline::addParallelFilters(std::vector<Filter*>& filters)
{
    ...
    unsigned long outputPipeStart = m_pipes.size();
    unsigned long inputPipeStart  = m_pipes.size() - filters.size();
    ...
    // insert the filters
    for (unsigned int i = 0; i < m_nrOfSources; ++i) {
        m_filterThreads.push_back(new FilterThread(m_pipes[i + outputPipeStart],
                                                   filters[i],
                                                   m_pipes[i + inputPipeStart],
                                                   m_bUseThreading));
...
void Pipeline::addMultiFilter(MultiFilter* pFilter)
{
    ...
    m_filterThreads.push_back(
        new MultiFilterThread(m_pipes[m_lastPipeID + 1],
                              pFilter,
                              m_pipes.end() - m_nrOfSources - 1,
                              m_pipes.end() - 1,
                              m_bUseThreading));
    ++m_lastPipeID;
...
{
    m_pipes.push_back(new Pipe(m_waitForSlowestFilter));
    m_filterThreads.push_back(new FilterThread(m_pipes[m_lastPipeID + 1],
                                               pFilter,
    ...
    if (m_bUseThreading) {
        ...
        for (auto thread : m_filterThreads)
            if (thread->isAlive())
                Log::getDefaultLog()
                    << Log::EL_ERROR
                    << "FilterThread is still alive in restart method!"
                    << Log::endl;
